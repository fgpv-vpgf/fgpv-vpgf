...
(() => {
        ...
        clickHandlerBuilder (clickEvent) {
            ...
            promises.push(new Promise((resolve) => {
                $.ajax({
                    ...
                    error: () => resolve({ magnetic: '', annChange: '', compass: '' })
                });
            }));
    ...
    function generateOutput(val, pt, dms, date, lang) {
        ...
        const coord = `<li>
                            ...
                            <div class="rv-subsection">
                                ...
                                <div>${trans.coordLat}${pt.y}</div>
                                <div>${trans.coordLong}${pt.x}</div>
        ...
        self.toggleDialogInfo(trans.title, `<ul class="rv-list">${coord}${utm}${nts}${alti}${mag}</ul>`);
    }
    ...
    function parseUtm(utm, pt) {
        // set zone
        const zone = utm.length > 0 ? utm[0].properties.identifier.padStart(2, '0') : '';

        // set the UTM easting/northing information using a geometry service
        const outPt = zone !== '' ? self.projectGeometry(pt, parseInt(`326${zone}`, 10)) : { x: '', y: '' };
