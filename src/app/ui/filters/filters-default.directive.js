...
(() => {
    ...
    function rvFiltersDefault($timeout, $q, stateManager, $compile, geoService, $translate, layoutService,
        detailService, $rootElement, $filter, keyNames, $sanitize, debounceService, configService) {
        ...
        function link(scope, el) { // scope, el, attr, ctrl) {
            ...
            function createTable(oLang) {
                ...
                function setkeytable() {
                    ...
                    $rootElement.find('.dataTables_scrollBody td').on('click', (event) => {
                        index = self.table.cell(event.currentTarget).index();
                    });

                    // TODO: event is fired up to 5 times. stopPropagation doesn't stop bubbling
                    $rootElement.find('.dataTables_scrollBody td').on('keydown', (event) => {
