...
function symbologyStack(ConfigObject, gapiService) {
    class SymbologyStack {
        ...
        constructor(
            ...
            this._isInteractive = symbols.length === 0 ? false : isInteractive; // the stack can only be interactive if there is at least one symbol

            // if no symbols or cover icon specified, exit
            if (symbols.length === 0 && !coverIcon) {
                return;
            }
            ...
            if (symbols.length > 0 && typeof symbols[0].image !== 'undefined') {
