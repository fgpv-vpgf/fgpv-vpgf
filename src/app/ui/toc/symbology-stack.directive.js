        ...
        if (self.block.selectedChanged) {
            self.block.selectedChanged.subscribe(val => {
                if (val) {
                    ...
                    const query = self.block.visibility ? (self.block.symbDefinitionQuery ? self.block.symbDefinitionQuery : undefined) : '1=2';
                    setTableDefinition(query, query);
                }
            });
        }
                    ...
                    $q.all(drawPromises).then(() => {
                        ...
                        self.symbology.stack.forEach(s => {
                            if (s.definitionClause) {
                                ...
                                const keys = Object.keys(self.toggleList);
                                keys.forEach(key => { if (self.toggleList[key].isSelected !== self.block.visibility) { self.onToggleClick(key, false); } });
                            }
                        });
                    });
                }
