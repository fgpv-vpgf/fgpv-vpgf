 ...
 *
 * ```html
 * <!-- `option` attribute specifies the name of the toggle; toggle's control object and its templateUrl are fetched from the layerItem directive -->
 * <rv-toc-entry-control option="settings"></toc-entry-control>
 *
 * <!-- `templateUrl` attribute specifies the templateUrl to be used for the control; it defaults to `entry-button.html`; another option is `entry-control-menu-item.html` -->
 * <rv-toc-entry-control option="metadata" templateUrl="menu-item"></rv-toc-entry-control>
 * ```
 *
...
function rvLegendControl(LegendElementFactory) {
    const directive = {
        ...
        templateUrl: (elm, attr) => {
            ...
            return templateUrl[attr.templateUrl || 'button'];
        },
        scope: {
            block: '=',
            name: '@'
