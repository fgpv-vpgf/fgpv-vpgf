        ...
        function toggleMetadata(entry, state) {
            ...
            if (typeof state === 'undefined' || state === false) {
                ...
                stateManager
                    ...
                    .catch(() => {
                        // create notification toast so the pane could be closed with toast's close button
                        const closeToast = $mdToast.simple()
                            .textContent($translate.instant('toc.error.resource.loadfailed'))
                            .action($translate.instant('contentPane.tooltip.close'))
                            .parent(layoutService.panes.metadata)
                            .position('bottom rv-flex');

                        $mdToast.show(closeToast)
                            .then(response => {
                                if (response === 'ok') {
                                    toggleMetadata(entry, true);
                                }
                            });
                    });
            } else {
