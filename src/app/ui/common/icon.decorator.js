(() => {
    ...
    function mdIconDirectiveDecorator($delegate, $timeout) {
        'ngInject';
        ...
        function decorateCompile(originalCompile) {
            return (...args) => {
                ...
                return (scope, el, attrs, ctrls) => {
                    ...
                    // $timeout needed since there is a delay md-icon has when adding svg elements
                    // see https://github.com/angular/material/blob/master/src/components/icon/js/iconDirective.js#L236
                    $timeout(() => {
                        el[0].childNodes[0].setAttribute('focusable', false);
                    });
                };
            };
        }
