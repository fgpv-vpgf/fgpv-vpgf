(() => {
    ...
    function geosearchService(stateManager, storageService, debounceService, geoSearch,
        $rootScope, geoService, gapiService) {
        ...
        function init() {
            ...
            const stopGeoServiceWatcher = $rootScope.$watch(() => geoService.isMapReady, isReady => {
                if (isReady) {
                    ...
                    // check if we enable visible extent filter
                    enableExtent();
                }
            });
         ...
         */
        function onExtentChange() {
            // check if we enable extent filter
            enableExtent();

            if (filters.extent === 'visible' && searchText.value !== '') {
                queryGeosearch();
            }
        }

        /**
         * Called on map extent changes to check if we are inside full extent. If not, disableSearch
         * visible extent checkbox because extent projection gives bad results. Only enable when current
         * extent is inside full extent.
         *
         * @function enableExtent
         * @private
         */
        function enableExtent() {
            const extent = geoService.mapObject.extent;
            const fullExtent = geoService.getFullExtent();

            if ((extent.xmin < fullExtent.xmin || extent.ymin < fullExtent.ymin) ||
                (extent.xmax > fullExtent.xmax || extent.ymax > fullExtent.ymax)) {
                filters.extentDisable = true;
                geoSearch.setExtent('canada');
            } else {
                filters.extentDisable = false;
                geoSearch.setExtent(filters.extent);
            }
        }
