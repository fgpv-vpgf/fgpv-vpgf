(() => {
    ...
    function geosearchService(stateManager, storageService, debounceService, geoSearch, geoService) {
         ...
         * @return  {Array} results the results from the query (emtpy array if there is no results)
         */
        function setResults(items) {
            const results = (items.results !== undefined) ?
        ...
        function toggleGeosearch() {
            ...
            // set onExtent change event when Geosearch open
            if (!eventExtent) {
                geoService.setExtentEvent(onExtentChange);
                eventExtent = true;
            }
        }
         ...
         */
        function closeGeosearch() {
            // TODO: when event will be return from geoAPI remove it when geoSearch is closed. At the same time, it will
            // solve the problem when we switch projection. For now, when we switch projection, the watch does not fire
            // so the extent-change does not work (same for filters)
            // eventExtent();
