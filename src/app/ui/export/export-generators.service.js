...
(() => {
    ...
    function exportGenerators($q, $filter, $translate, $templateCache, gapiService, configService, graphicsService,
        ...
        function mapLocalGenerator(exportSize) {
            const mapInstance = configService.getSync.map.body;

            const localGeneratorPromise =
                    gapiService.gapi.mapPrint.printLocal(mapInstance, exportSize);
        ...
        function mapServerGenerator(exportSize, showToast) {
            const { map: { body: mapInstance }, services: { exportMapUrl } } = configService.getSync;
        ...
        function scalebarGenerator(exportSize) {
            ...
            const mapInstance = configService.getSync.map.body;
            const scale = gapiService.gapi.mapManager.getScaleRatio(mapInstance, containerWidth);
...
<<<<<<< 2a93a037c9b52c2ac1174a397d4a313ae97c99a2
            const mapInstance = configService.getSync.map.body;
            const rotation = gapiService.gapi.mapManager.getNorthArrowAngle(mapInstance) - 180;
=======
