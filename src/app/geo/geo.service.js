...
(() => {
    ...
    function geoService(layerTypes) {
         ...
         * @param {number} order an optional index indicating at which position the layer was added to the map
         * (if supplied it is the caller's responsibility to make sure the layer is added in the correct location)
         */
        function registerLayer(layer, config, attribs, order) {
            ...
            if (order === undefined) {
                order = service.layerOrder.length;
            }
            service.layerOrder.splice(order, 0, layer.id);
        }
