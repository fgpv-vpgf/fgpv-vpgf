...
function LegendBlockFactory($q, Geo, common, layerRegistry, gapiService, configService) {
    ...
    class LegendBlock {
        ...
        get templateUrl () {               return this.blockType; }
    ...
    class LegendNode extends LegendEntry {
        ...
        get templateUrl () {
            const stateToTemplate = {
                ...
                'rv-loaded': () => super.templateUrl,
                'rv-refresh': () => super.templateUrl,
                'rv-error': () => 'error'
            };
    ...
    // LegendGroup needs the dynamic layer record root proxy to know when the layer fails and display the error templateUrl when this happens
    // If the layer fails on initial loading, there are no children to indicate the error state, so the error templateUrl must be displayed at the root of the dynamic record
