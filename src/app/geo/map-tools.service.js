(() => {
    ...
    function mapToolService(geoService, gapiService) {
        ...
        */
        function northArrow() {
            const mapPntCntr = geoService.mapObject.extent.getCenter();
            const mapScrnCntr = geoService.mapObject.toScreen(mapPntCntr);
            const wkid = geoService.mapObject.spatialReference.wkid;

            let screenX = null;
            let angleDegrees = null;
            let rotationAngle = null;

            if (wkid === 3978) { // lambert
                ...
                angleDegrees = 270 - gapiService.gapi.mapManager.getNorthArrowAngle(geoService.mapObject);
                ...
                screenX = mapScrnCntr.x + (Math.sin((90 - angleDegrees) * 0.01745329252) * z);

            } else if (wkid === 102100) { // mercator
