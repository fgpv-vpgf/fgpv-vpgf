...
(() => {
    ...
    function autolink() {
        ...
        function autolink(items, options = {}) {
            ...
            function process(item) {
                ...
                const isHtml = /<(?=.*? .*?\/ ?>|br|hr|input|!--|wbr)[a-z]+.*?>|<([a-z]+).*?<\/\1>/.test(item); // https://regex101.com/r/cX0eP2/1
                const opts = angular.extend(defaultOptions, options);

                return isHtml ? linkifyHtml((item || '').toString(), opts) : linkifyStr((item || '').toString(), opts);
            }
        }
    }
    ...
    function picture() {
        ...
        /**
         * Picture filter replace a href by image tag or/and lighbox open on click.
         *
         * @function picture
