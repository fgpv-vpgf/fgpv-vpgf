...
(() => {
    ...
    function focusService($rootElement, $rootScope, $mdDialog, storageService) {
        ...
        let focusStatus;
        let focusSelector = `a[href], area[href], input:not([disabled]), select:not([disabled]),
         ...
         */
        function findNextFocusable() {
            let element = $rootElement.find(focusSelector).last();
            let externalElement = $();
        ...
        function findPrevFocusable() {
            let element = $rootElement.find(focusSelector).first();
            let externalElement = $();
        ...
        function _activeRootFocusoutCB(event) {
            const firstFocusable = $rootElement.find(focusSelector).first();
            const lastFocusable = $rootElement.find(focusSelector).last();
        ...
        function _waitingKeydownCB(event) {
            ...
            if (focusStatus === 'WAITING' && event.key === 'Enter') {
                event.preventDefault();
            }

            if (focusStatus === 'ACTIVE' && event.key === 'Enter' && event.shiftKey) {
                setInactive(false);

            } else if (focusStatus === 'WAITING' && event.key === 'Enter' && !event.shiftKey) {
                setStatus('ACTIVE');
                dlg.hide();

            } else if (focusStatus === 'WAITING' && event.key === 'Tab') {
                setInactive(event.shiftKey);
            }

            // detect focus direction is reversed
            isForward = event.key === 'Tab' ? !event.shiftKey : isForward;
        }
