...
function shellService(common, errorService) {
    ...
    function setLoadingFlag(item) {
        /* console.log(
            ...
            item.id,
            item.initDelay,
            Object.values(service.loadingProcesses).map(p => ({ id: p.id, state: p._state })),
            service.isLoading
        ); */
        ...
        process._initDelayHandle = common.$timeout(() => {
            ...
            process._messageDelaylHandle = common.$timeout(() => {
                /* console.log(
                    `[]- process interval ${process.id}, ${process.messageDelay} ${process.initDelay} ${
                        process.message
                    }`
                ); */
     ...
     * @param {number} [delay=0] delay before the process should be cleared; this is useful if a process tends to yo-yo between being active and inactive
     */
    ...
    function _clearProcess(id) {
        ...
        // remove the corresponding notification if display or in the queue
