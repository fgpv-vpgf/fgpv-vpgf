...
function runBlock($rootScope, $rootElement, $q, globalRegistry, reloadService, events, configService,
    ...
    /**
     * A helper function to create ids for templateUrl elements inside directives.
     ...
     * Adding `{{ ::$root.uid($id) }}` inside a templateUrl will return a `{appid}-{scopeid}` string. If this used several times inside a single templateUrl, the same id is returned, so you don't have to store it to reuse. Don't forget a one-time binding.
     * @function uid
     * @private
