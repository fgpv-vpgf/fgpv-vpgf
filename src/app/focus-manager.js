...
((RV, jQuery) => {
    ...
    const history = [];
    // list of element pairs which focus moves between
    let linkedList = [];
    ...
    class Viewer {
        ...
        setStatus (status) {
            ...
            if (this.mdDialog && this.status === statuses.WAITING) {
                this.mdDialog.hide();
            // set tabindex for all elements of the viewer except the map so that when a user tabs
            ...
            } else if (status === statuses.WAITING) {
                this.rootElement
                    .find(focusSelector)
                    .not('.rv-esri-map')
                    .attr('tabindex', -1);
        ...
        if (viewerActive) {
            ...
            if (event.which === 9) { // tab key
                if (keys[27]) { // escape key
                    ...
                    // if focus has been changed manually, prevent browser from changing the focus
                    if (shiftFocus(!event.shiftKey)) {
    ...
    function disableCommonPrototypes(funcName) {
        return (function () {
            var originalFunc = jQuery.Event.prototype[funcName];
