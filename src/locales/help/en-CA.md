...
First open the layers panel by selecting the layer button (shown above in red). You will then see the basemap selector appear to the left of the layer button (shown above in blue).
...
| ![](layer_settings/settings.png) | Settings | Opens slideout panel where opacity amount and bounding box visibility can be toggled |
