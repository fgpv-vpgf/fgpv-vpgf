...
* the original id value is copied in a newly created property IDgeoJSON of the properties object
* and the existing id value is replaced by an autogenerated number.
* Features without existing id from that same dataset will get a new properties IDgeoJSON
...
function assignIds(geoJson) {
    ...
    if (empty) {
        geoJson.features.forEach(function (val) {
            delete val.properties.IDgeoJSON;
        });
    }
}
