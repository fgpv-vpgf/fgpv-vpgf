...
$.getScript('../../../../rv-main.js', function () {
        ...
        legendPopulated.then(value => {
            ...
            $("#ItemToggleOpacity").click(function () {
                if (legendItem._availableControls.includes('opacity')) {
                    ...
                    else if (legendItem.opacity === 0.3) {
                        legendItem.opacity = 1;
                    }
                    else {
                        $('#ItemToggleOpacity').css('background-color', 'red');
                        console.log('Opacity is undefined for an item with available controls. If not, it may have been changed from the settings panel.');
                    }
                }
            });
            ...
            $("#GroupToggleOpacity").click(function () {
                if (legendGroup._availableControls.includes('opacity')) {
                    if (legendGroup.opacity === 1) {
                        legendGroup.opacity = 0.3;
                    }
                    else if (legendGroup.opacity === 0.3) {
                        legendGroup.opacity = 1;
                    }
                    else {
                        $('#GroupToggleOpacity').css('background-color', 'red');
                        console.log('Opacity is undefined for a group with available controls. If not, it may have been changed from the settings panel.');
                    }
                }
            });
    ...
    `);
    const mapInstance = new RZ.Map(document.getElementById('fgpmap'), '../test-legend-one.json');
});
