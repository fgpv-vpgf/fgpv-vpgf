...
$.getScript('../../../../rv-main.js', function () {
    RZ.mapAdded.subscribe(mapi => {
        ...
        let legendPopulated = new Promise(function (resolve, reject) {
            const childrenLoaded = setInterval(() => {
                if (mapi.ui.configLegend.children.length === 2 && mapi.ui.configLegend.children[0].children.length === 4 && mapi.ui.configLegend.children[1].children.length === 3) {
    ...
    `);
    const mapInstance = new RZ.Map(document.getElementById('fgpmap'), '../test-legend-three.json');
});
