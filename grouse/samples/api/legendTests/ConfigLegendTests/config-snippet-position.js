...
$.getScript('../../../../rv-main.js', function () {
    RAMP.mapAdded.subscribe((mapi) => {
        ...
        legendPopulated.then((value) => {
            ...
            $('#GetSnippet').click(function () {
                let children = '';
                legend.configSnippets.forEach((child) => {
                    children += JSON.stringify(child, null, 4) + '\n\n\n\n';
                });
    ...
    $('#main').append(`
        <div id="fgpmap" style="height:700px; width:80%; margin-left:10px" class="column" rv-langs='["en-CA", "fr-CA"]' rv-service-endpoint="http://section917.canadacentral.cloudapp.azure.com/""></div>
    `);
