export const pinImg = `data:image/png;base64,`;

export const picHTML1 = `<li class="rv-basemap-list-item" style="height: 175px;">
                         <div style="position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden; height: 100%">
                         <div style="max-height: 175px; max-width: 350px; text-align: center;"><img alt="" `;

export const picHTML2 = `/></div><div class="rv-basemap-footer"><span style="text-overflow:ellipses">`;

export const HTMLmiddle = `</span></div><button `; 

export const HTMLmiddle2 = `class="rv-body-button rv-button-square md-button md-ink-ripple" type="button" aria-label="bookmark" `;

export const HTMLend = `<div class="md-ripple-container" style=""></div></button></div></li>`;

export const noPicHTML1 = `<li class="rv-basemap-list-item" style="height: 50px;">
                            <div style="position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden; height: 100%">
                            <div style="max-height: 50px; max-width: 350px; text-align: center;"></div>
                            <div class="rv-basemap-footer"><span>`;


