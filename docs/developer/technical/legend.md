...
__Legend__ is a list of (legend) blocks displayed in the main panel (toc pane - table of content). When opened, the legend pane is titled "Layers" - due to popular user demand.
...
__Legend__ is a list of icons/images with corresponding labels representing data displayed by a certain layer. _Devs_ call it the `symbology stack`.
...
__Structured__ legend lets the config author to mess with the ordering of legend blocks, their names, specify custom symbology stacks, put things into regular groups or visibility sets, hide layer from the legend, and have a single legend block controlling several layers. Lots of power options, plenty of opportunities to screw up the config. All this should be used for thematic maps, maps that tell a certain story with the data.
...
It should be pointed out the __autogenerated__ legend - after it's generated during the config parse - is technically a __structured__ legend. It uses a subset of elements which can be used in the __structured__ legend.
...
__Info section__ is a piece of static content (unless it's a .gif) which can be added between any other legend blocks. There are four types:
...
Note: `coverIcon` option is not yet implemented.

`controlledIds` options lets a single legend block to control properties of several layers. This is useful if you want to bundle some datasets.
...
Note: the `description` option will available in 2.1 adds an optional, plain text description to the top of the symbology stack.


#### Entry Group (config) or Legend Group (code)

These let you group things into collapsible sections. Anything can be put inside a group, other groups including (group nesting). Nesting them more than ten levels deep will not produce any changes in the UI - it's a flat list from there on.
...
When creating a Legend Node block (`_makeLegendBlock`), first the appropriate blueprints are found - there must be a single main blueprint and zero or move controlled blueprints (`controlledIds` option from the config). Then, legend block is mapped to the entry type:
...
The controlled layers are processed in a similar passion to controlled layers on a `LegendNode` with huge difference - controlled `ProxyWrapper`s are actually turned into normal legend blocks which are added to the group as usual. These blocks are marked as `controlled` which precludes them from being rendered.
