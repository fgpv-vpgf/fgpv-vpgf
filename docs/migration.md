...
We've taken a balanced approach to introducing these changes so that migration from RAMP version 2 to version 3 is relatively pain free. The RAMP version 2 API is now deprecated, but continues to be available until the next major release. All RAMP v2 plugins have been ported to v3 as well.
...
<p class="warning">
  The deprecated v2 API will be removed in the next major release, so its a good idea to start using the new v3 API as soon as possible.
  <br><br>
