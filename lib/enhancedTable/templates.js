...
exports.DETAILS_TEMPLATE = function (oid) {
    return "<button ng-controller='DetailsAndZoomCtrl as ctrl' ng-click='ctrl.openDetails(" + oid + ")' md-ink-ripple class='md-icon-button rv-icon-16 rv-button-24 md-button ng-scope enhanced-table-details' aria-label=\"{{ 't.detailsAndZoom.details' | translate }}\">\n        <md-icon md-svg-src=\"action:description\" aria-hidden='false' class='ng-scope' role='img'>\n            <md-tooltip  md-direction=\"top\">{{ 't.detailsAndZoom.details' | translate }}</md-tooltip>\n        </md-icon>\n    </button>";
};
exports.ZOOM_TEMPLATE = function (oid) {
    return "<button ng-controller='DetailsAndZoomCtrl as ctrl' ng-click='ctrl.zoomToFeature(" + oid + ")'  md-ink-ripple class='md-icon-button rv-icon-16 rv-button-24 md-button ng-scope enhanced-table-zoom' aria-label=\"{{ 't.detailsAndZoom.zoom' | translate }}\">\n        <md-icon md-svg-src=\"action:zoom_in\" aria-hidden='false'>\n            <md-tooltip  md-direction=\"top\">{{ 't.detailsAndZoom.zoom' | translate }}</md-tooltip>\n        </md-icon>\n    </button>";
};
...
exports.DATE_FILTER_TEMPLATE = function (value, isStatic) {
    if (isStatic === true) {
...
exports.TEXT_FILTER_TEMPLATE = function (value, isStatic) {
...
exports.PRINT_TABLE = function (title, cols, rws) {
    // make headers with the column names of the currently displayed columns
    var headers = "";
    var columnNames = Object.keys(cols).map(function (column) { return cols[column]; });
    ...
    var columns = "<thead><tr>" + headers + "</tr></thead>";
    // make rows
    // make sure row attributes are only pushed for columns that are currently displayed.
    var rows = "<tbody>" + rws.map(function (rowAttributes) {
        var eachRow = Object.keys(cols).map(function (attribute) { return rowAttributes.data[attribute]; });
        return "<tr>" + eachRow.map(function (r) {
