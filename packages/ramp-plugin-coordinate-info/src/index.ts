...
export default class CoordInfo {
  ...
  init(api: any) {
    ...
    this.button = this.api.mapI.addPluginButton(
      CoordInfo.prototype.translations[this._RV.getCurrentLang()]
        .coordButtonLabel,
  ...
  onMenuItemClick() {
    ...
    return () => {
      ...
      if (typeof this.handler === "undefined") {
        ...
        const activeNode =
          this.api.mapDiv[0].getAttributeNode("coord-info-active");
   ...
   */
  toggleActive() {
    this.handler = this.api.click.subscribe((clickEvent) =>
    ...
    promises.push(
      new Promise((resolve) => {
        $.ajax({
          ...
          dataType: "jsonp",
          success: (data) =>
            resolve(data.altitude !== null ? data.altitude : 0),
        });
      })
    );
  ...
  parseNTS(nts) {
    ...
    const nts250 =
      nts.length > 0
        ? `${nts[0].properties.identifier}-${nts[0].properties.name}`
        : "";

    // set 50k
    const nts50 =
      nts.length > 1
        ? `${nts[1].properties.identifier}-${nts[1].properties.name}`
        : "";
