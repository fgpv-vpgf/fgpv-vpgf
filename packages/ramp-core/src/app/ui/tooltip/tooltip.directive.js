 ...
 *
 */
angular.module('app.ui').directive('rvTooltip', rvTooltip);
...
function rvTooltip($q, debounceService) {
    const directive = {
        ...
        link: (scope, el) => {
            ...
            const debouncedUpdateDimensions = debounceService.registerDebounce(
                (dimensions) => {
            ...
            scope.$watch(watchBBoxChanges, (newDimensions) => debouncedUpdateDimensions(newDimensions), true); // the last argument (true) is set for $watch to do object equality comparison instead of reference equality
