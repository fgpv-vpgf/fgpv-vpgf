 ...
 */
angular.module('app.geo').factory('geoService', geoService);
...
function geoService(
    ...
    class GeoServiceInterface {
        ...
        get map() {
            return configService.getSync.map.instance;
        }
                            ...
                            if (orderdBookmarkIds.length !== 0) {
                                ...
                                for (let i = 0; i < orderdBookmarkIds.length; i++) {
                                    if (orderdBookmarkIds[i] === layer.id) {
                                        ...
                                        layer.id.split('.')[0] === 'rcs' &&
                                        orderdBookmarkIds[i].split('.')[1] === layer.id.split('.')[1]
                                    ) {
        ...
        validateExtent(factor = 1) {
            return this.map
                .enhanceConfigExtent(configService.getSync.map.selectedBasemap.full)
                .expand(factor)
                .contains(this.map.extent);
        }
        ...
        reloadLayer(l) {
            layerRegistry.reloadLayer(l);
        }
        snapshotLayer(l) {
            layerRegistry.snapshotLayer(l);
        }

        get defaultExtent() {
            return configService.getSync.map.selectedBasemap.default;
        }
        get fullExtent() {
            return configService.getSync.map.selectedBasemap.full;
        }
        get maximumExtent() {
            return configService.getSync.map.selectedBasemap.maximum;
        }
        get currentExtent() {
