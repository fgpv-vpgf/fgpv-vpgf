...
import EnhancedTable from 'ramp-plugin-enhanced-table';
    ...
    loader() {
        ...
        Object.keys(AUTOLOAD_PLUGINS).forEach((pk) => {
            ...
            if (!pN && loadFeatures[pk]) {
                console.warn(
                    `The plugin ${pk} was loaded automatically. This functionality may be removed in a future release. Please load this plugin on the host page instead (see ramp documentation) or add 'no-${pk}' to rv-plugins to stop this plugin from being autoloaded.`
                );
            } else if (pN === `no-${pk}`) {
