...
export function setUpNumberFilter(
    colDef: any,
    isItStatic: boolean,
    defaultValue: any,
    gridOptions: any,
    panelStateManager: any
) {
...
export function setUpTextFilter(
    colDef: any,
    isStatic: boolean,
    searchStrictMatchEnabled: boolean,
    defaultValue: any,
    map: any,
    panelStateManager: any
) {
    ...
    defaultValue =
        panelStateManager.getColumnFilter(colDef.field) !== undefined
...
export function setUpSelectorFilter(
    colDef: any,
    isItStatic: boolean,
    defaultValue: any,
    gridOptions: any,
    ...
    let value =
        panelStateManager.getColumnFilter(colDef.field) !== undefined
            ? panelStateManager.getColumnFilter(colDef.field) === null
                ? undefined
                : panelStateManager.getColumnFilter(colDef.field)
            : defaultValue;
...
export class NumberFloatingFilter {
    ...
    get preLoadedValue(): any {
        ...
        const defaultMinVal =
            this.params.defaultValue === undefined || this.params.defaultValue.split(',')[0] === ''
                ? null
                : Number(this.params.defaultValue.split(',')[0]);

        const defaultMaxVal =
            this.params.defaultValue === undefined || this.params.defaultValue.split(',')[1] === ''
                ? null
    ...
    onMinInputBoxChanged() {
        if (this.minFilterInput.value === '') {
            ...
            console.error(
                `${this.minFilterInput.value} is not a valid filter input for numberic column "${this.params.currColumn.field}"`
            );
    ...
    onMaxInputBoxChanged() {
        if (this.maxFilterInput.value === '') {
            ...
            console.error(
                `${this.maxFilterInput.value} is not a valid filter input for numberic column "${this.params.currColumn.field}"`
            );
...
export class DateFloatingFilter {
    init(params: any) {
        ...
        this.scope.minChanged = () => {
            // save value on panel reload manager
            let key = this.params.currColumn.field + ' min';
            this.params.panelStateManager.setColumnFilter(
                key,
                this.scope.min !== null ? this.scope.min.toString() : null
            );
            this.onFloatingFilterChanged({ model: this.getModel() });
        };
        ...
        this.scope.maxChanged = () => {
            ...
            this.params.panelStateManager.setColumnFilter(
                key,
                this.scope.max !== null ? this.scope.max.toString() : null
            );
            this.onFloatingFilterChanged({ model: this.getModel() });
        };
    ...
    preLoadedValue(): void {
        const defaultMinVal =
            this.params.value !== undefined && this.params.value.split(',')[0] !== ''
                ? new Date(this.params.value.split(',')[0])
                : null;

        const defaultMaxVal =
            this.params.value !== undefined && this.params.value.split(',')[1] !== ''
                ? new Date(this.params.value.split(',')[1])
                : null;
...
export class SelectorFloatingFilter {
    init(params: any) {
        ...
        function getDistinctRows(rowData) {
            let distinctRows = {};
            rowData.filter((row) => {
                return distinctRows.hasOwnProperty(row[params.currColumn.headerName])
                    ? false
                    : (distinctRows[row[params.currColumn.headerName]] = true);
            });
            return distinctRows;
        }
    ...
    preLoadedValue(): void {
        const reloadedVal =
            this.params.panelStateManager.getColumnFilter(this.params.currColumn.field) === null
                ? ''
    ...
    getModel(): any {
        let optionsList =
            this.scope.selectedOptions.length === 1 && this.scope.selectedOptions[0] === ''
                ? []
