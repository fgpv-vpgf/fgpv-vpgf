...
class LayerRecord extends root.Root {
    ...
    get initLoadDone() {
        return this._sawLoad && this._sawRefresh;
    }
    ...
    onLoad() {
        ...
        this.defaultVisibility = this._layer.defaultVisibility;
        ...
        if (!hasVisibleState && this.defaultVisibility !== undefined) {
            ...
            this.config.state.visibility = this.defaultVisibility;
            ...
            if (!this.config.source.state) {
                ...
                this.config.source.state.visibility = this.defaultVisibility;
            }
        }
    ...
    findZoomScale(lods, scaleSet, zoomIn = true) {
        ...
        const scaleLod = modLods.find((currentLod) =>
            zoomIn ? currentLod.scale < scaleSet.minScale : currentLod.scale > scaleSet.maxScale
        );
    ...
    setMapScale(map, lod, zoomIn, positionOverLayer = true) {
        ...
        if (zoomIn && positionOverLayer) {
            ...
            const reprojLayerFullExt = this._apiRef.Map.Extent(
                gextent.x0,
                gextent.y0,
                gextent.x1,
                gextent.y1,
    ...
    constructor(layerClass, apiRef, config, esriLayer, epsgLookup) {
        ...
        this._layerPassthroughBindings.forEach(
            (bindingName) => (this[bindingName] = (...args) => this._layer[bindingName](...args))
        );
