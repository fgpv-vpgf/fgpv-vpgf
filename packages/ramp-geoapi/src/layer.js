...
function validateGeoJson(geoJson) {
    ...
    if (geomType === 'GeometryCollection') {
        ...
        // scan rest of file to ensure geometry type is consistent
        if (geoJson.features.length > 1) {
            for (let i = 1; i <= geoJson.features.length; i++) {
                if (geoJson.features[i].geometry.type !== geomType) {
...
function findNumbericalCandidates(rows, range) {
    ...
    const candidates = fields.filter((field, index) =>
        rows.every(
            (row, rowIndex) =>
 ...
 * @param {Object} geoJson           layer data in geoJson format
 */
function cleanUpGeomCollection(geoJson) {
    const geoms = geoJson.features[0].geometry.geometries;
    ...
    if (geoms.length === 1) {
        ...
        let combined = [];
        geoms.forEach((geom) => {
            combined.push(geom.coordinates);
        });
    ...
    return (geoJson, opts) => {
        ...
        if (opts) {
            ...
            if (opts.targetSR) {
                ...
                console.warn(
                    'Suggest using option targetSR instead of targetWkid, as unofficial support for targetWkid may end at any time'
                );
                targetSR = {
        ...
        return srcLookup.lookupPromise.then(() => destLookup.lookupPromise).then(() => buildLayer());
    };
}
...
function createFeatureRecordBuilder(esriBundle, geoApi, classBundle) {
     ...
     */
    return (config, esriLayer, epsgLookup) => {
        return new classBundle.FeatureRecord(
            esriBundle.FeatureLayer,
